(function($){
	$.lrc = {
		...
		format: '<li><span>{html}</span></li>', /* 模板 */
		prefixid: 'lrc', /* 容器ID */
		hoverClass: 'hover', /* 选中节点的className */
		hoverTop: 6.67, /* 当前歌词距离父节点的高度 */
		...
		start: function(txt, callback) {
			...
			// 	this.fontsize = $(document.body)[0].clientWidth / 30;
			// }
			...
			if(this.list.length > 0) {
				...
				for(var i = 0; i < this.list.length; i++){
					if(i == 0){
						...
						var tmplrc = (this.list[i][1] == '') ? '......' : this.list[i][1];
						html += this.format.replace(/\{html\}/gi, tmplrc);
					}
				}
 
				/* 赋值到指定容器 */

				if($('#'+this.prefixid+'_list').children().length == 0){
					$('#'+this.prefixid+'_list').html(html).animate({ marginTop: 0 }, 100).show();
				}
				/* 隐藏没有歌词的层 */
				$('#'+this.prefixid+'_nofound').hide();
				if(callback == 'init') return;
				/* 定时调用回调函数，监听歌曲进度 */
		...
		jump: function(duration) {
			...
			tmp = tmpobj.next().offset().top-tmpobj.parent().offset().top - (this.hoverTop*this.lineheight);
			tmp = tmp * -1;
			tmpobj.parent().animate({marginTop: tmp + 'px'}, this.interval*1000);
			//this.animata(tmpobj.parent()[0]).animate({marginTop: tmp + 'px'}, this.interval*1000);
			if (tmpobj[0].offsetWidth < tmpobj.children()[0].offsetWidth) {
				tmp = tmpobj[0].offsetWidth - tmpobj.children()[0].offsetWidth;
				tmpobj.animate({textIndent: tmp}, (this.list[pivot+1][0] - this.list[pivot][0]) * 1000, function() {
